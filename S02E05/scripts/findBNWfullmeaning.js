...
async function findRelevantParagraph(paragraphsWithBNW) {
    try {
        const results = [];
        
        // Analyze all paragraphs and collect their relevance scores
        for (const para of paragraphsWithBNW) {
            const response = await openai.chat.completions.create({
                model: "gpt-4o-mini",  // Using gpt-4o-mini
                messages: [
                    {
                        role: "system",
                        content: "Przeanalizuj tekst i oceń w skali 0-10, na ile prawdopodobne jest że zawiera on odpowiedź na pytanie o to, co oznacza akronim BNW. Dodatkowo krótko uzasadnij ocenę. Odpowiedz w formacie: 'OCENA: [liczba 0-10]\nUZASADNIENIE: [krótkie uzasadnienie]'"
                    },
                    {
                        role: "user",
    ...
    try {
        const response = await openai.chat.completions.create({
            ...
            messages: [
                {
                    role: "system",
                    content: "Odpowiedz jednym krótkim zdaniem na pytanie, co oznacza akronim BNW, bazując na podanym tekście."
                },
                {
                    role: "user",
                    content: `Kontekst:\n${paragraph.content}\n\nPytanie: Od czego pochodzą litery BNW w nazwie nowego modelu językowego?`
                }
            ],
            temperature: 0.1,
            max_tokens: 100
...
async function processQuestion() {
    try {
        ...
        // Filter paragraphs that contain word "BNW"
        const paragraphsWithBNW = [];
        for (const file of paragraphFiles) {
            const content = await fs.readFile(path.join(paragraphsDir, file), 'utf8');
            
            if (content.includes('BNW')) {
                const title = content.split('\n')[0]; // First line is the title
                paragraphsWithBNW.push({
                    title,
                    content,
                    file
                });
        ...
        console.log(`Found ${paragraphsWithBNW.length} paragraphs containing "BNW"`);

        // Find the most relevant paragraph
        const relevantParagraph = await findRelevantParagraph(paragraphsWithBNW);
        ...
        const answerPath = path.join(answersDir, 'answer05.json');
